    ...
    if choice == "1":
        while True:
            ...
            if choice1 == "1":
                ...
                output_dir = str(input("Specify the storage path (just type the name to save it in the same location as the program): "))
                
                while True:
                    answer = str(input(f"id: {id}, name: {name}, storage path: {output_dir} Is this correct? (Y/N): "))
                    if answer == "Y":
                        ...
                        # Add delay entry
                        config["delays"][identifier] = current_count # default delay logic
                ...
                try:
                    idx_to_del = int(input("Enter the number of the channel to delete: ")) - 1
                ...
                print("Current channel list:")
                for idx, channel in enumerate(config["channels"], start=1):
                    print(f"{idx}. id: {channel['id']}, name: {channel['name']}, recording status: {'On' if channel.get('active', 'on') == 'on' else 'Off'}")

                try:
                    idx_to_toggle = int(input("Enter the number of the channel to toggle recording status: ")) - 1
        ...
        while True:
            ...
            if choice2 == "1":
                print(f"The current number of recording threads is {config.get('stream_segment_threads', 2)}.")
                print("Recommended 2~4 threads, 2 threads for low-end systems / 4 threads for high-end systems")
                try:
                ...
                print(f"The current broadcast rescan interval is {config.get('timeout', 60)} seconds.")
                try:
                    new_timeout = int(input("Enter the rescan interval to change (in seconds): "))
        ...
        while True:
            ...
            if choice3 == "1":
                ...
                save_config(config)
                print(f"HEVC encoding has been {'enabled' if hevc['enable'] else 'disabled'}.")
                ...
                print("Options: ultrafast (rec), superfast, veryfast, faster, fast, medium")
