...
2. 输入 **主播唯一 ID**。
   - 它是 CHZZK 频道地址最后面的英文字母和数字。
   - 示例：如果地址是 `https://chzzk.naver.com/abc1234`，则 `abc1234` 是 ID。

3. 输入 **主播名称**。
   - 也可以使用你自己容易识别的昵称。

4. 输入 **保存路径**。
   - 如果什么都不输入直接按 Enter，文件会保存到程序文件夹中。

5. 如果输入的信息正确，请输入 `Y`。

> **提示：** 要录制需要成人认证的直播，需要在设置菜单的 **5. Cookie Settings** 中输入 Cookie 值（NID_AUT、NID_SES）。
...
**Q. 录制的文件在哪里？**
A. 如果设置时没有另外指定保存路径，文件会保存在项目文件夹中。
