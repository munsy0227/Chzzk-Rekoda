...
Windows 執行 `chzzk_gui.bat`，macOS/Linux 執行 `./chzzk_gui`。僅選擇 GUI 時安裝 Qt，現有 `settings` 與 `chzzk_record` CLI 仍可使用。
...
- 預設關閉視窗後繼續在系統匣錄製，可透過圖示還原視窗或**安全結束**。應用程式設定中可關閉此行為；無系統匣時關閉視窗會安全結束。防止 CLI/GUI 重複錄製及設定覆寫衝突。
- 儲存的錄製選項套用至新的錄製工作。DNS 與檔案記錄變更需停止並重新啟動錄製器。

GUI 需要桌面顯示環境與 FFmpeg。瀏覽器登入會開啟新瀏覽器並自動匯入 Cookie。實際瀏覽器登入與 Windows/macOS GUI 行為仍需於對應平台驗證。
...
- 每個已儲存影片或分段旁提供包含原始標題的 UTF-8 `.txt` 檔案。長檔名仍縮短並加入雜湊，TXT 保留完整原文。
- GUI 優先使用 `font/NotoSansKR-VariableFont_wght.ttf`，若不存在則在 `font` 下的 Noto CJK 字型套件中尋找 `NotoSansKR-VF.ttf` 或 `NotoSansCJKkr-VF.ttf`。目前字型套件的授權見 `font/02_NotoSansCJK-TTF-VF/LICENSE`。CLI 主選單10/11與頻道選單5也可修改新錄製設定。
