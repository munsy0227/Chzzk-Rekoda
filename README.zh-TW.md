...
Windows 按兩下 **`chzzk_gui.vbs`** 即可不顯示 CMD 視窗啟動，`chzzk_gui.bat` 也呼叫此啟動器。macOS/Linux 使用 `./chzzk_gui`。僅選擇 GUI 時安裝 Qt。
...
- GUI 使用 `font/02_NotoSansCJK-TTF-VF/Variable/TTF/Subset/NotoSansKR-VF.ttf`，僅保留所需的韓文字型及發行套件的 `LICENSE`。
