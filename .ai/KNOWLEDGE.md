...
- 5개 언어 README는 상단 GUI/CLI 화면 → 기존 소개와 Android 튜토리얼 링크 → GUI 첫 설정·리본 설정 → 설치 → CLI → FAQ 순서다. 사용자가 원래 소개 문구와 두 튜토리얼 링크를 유지하도록 요청했으므로 이후 구성 변경에서도 보존한다. 사용자 제공 화면은 `assets/screenshots/gui-recording.jpg`, `cli-recording.jpg`를 공용으로 사용한다. 사용자의 `gui` → `main` 병합 계획에 맞춰 ZIP은 `main.zip`, 저장소 링크는 루트 URL, clone은 기본 브랜치를 사용하도록 안내한다. 실제 병합 완료를 의미하지 않는다. [README 구성 및 검증](worklogs/2026-09-12-readme-gui-first.md)을 참고한다.
...
- `recorder_bridge.py:JsonBridge.display()`는 실제 `channel_progress`가 남아 있는 채널에만 종료 중 `stopping`을 보낸다. 녹화 대기 채널은 `idle`, 비활성 채널은 `inactive`로 유지하며, 비활성으로 바꿨더라도 파일 정리가 남아 있으면 진행 상태를 우선한다. `chzzk_record.py:record_stream()`은 화질 조회 실패 시 방송 상태를 다시 확인하고, 종료된 방송은 대기하며 아직 OPEN이면 채널 이름을 포함한 경고 후 설정된 간격으로 재시도한다. 예상 가능한 조회 실패에 Traceback을 남기지 않으며, `prepare_recording_quality()`는 중지/작업 취소 시 조회 프로세스 정리를 기다린다. [중지 상태와 화질 조회 수정](worklogs/2026-09-11-stop-status-quality.md)을 참고한다.
